...
export default function CustomersPage() {
  ...
  const fetchCustomers = async () => {
    try {
      const data = await masterAPI.getCustomersWithOutstanding(1, 100);
      if (data && data.success) {
        const customersData = Array.isArray(data.data) ? data.data : [];
        setCustomers(customersData);
      } else {
        showError((data && data.message) || 'Failed to fetch customers');
      }
    } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        success(editingId ? 'Customer updated successfully!' : 'Customer created successfully!');
        setShowModal(false);
  ...
  const filteredCustomers = customers.filter(customer => {
    ...
    const phone_no = customer.phone_no?.toLowerCase() || '';
    ...
    <>
      ...
      <div className="p-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Master Customers</h1>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredCustomers.map((customer) => (
                    <tr key={customer.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4">
                        <span className="text-sm text-gray-600">{customer.phone_no || '-'}</span>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 text-right">
                        <span className={`text-sm font-medium ${
                          (customer.outstanding || 0) > 0 ? 'text-red-600' : 'text-green-600'
                        }`}>
                          Rp {(customer.outstanding || 0).toLocaleString('id-ID')}
                        </span>
                      </td>
