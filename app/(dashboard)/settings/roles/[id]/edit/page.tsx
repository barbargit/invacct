...
export default function EditRolePage() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch role details and modules in parallel
      const [roleRes, modulesRes] = await Promise.all([
        userAPI.getRoleById(parseInt(id)),
        userAPI.getModules()
      ]);
      ...
      if (roleRes.success && roleRes.data) {
        setFormData({
          ...
          is_active: roleRes.data.is_active !== false,
        });
      } else {
      ...
      const permRes = await userAPI.getRolePermissions(parseInt(id));
      if (permRes.success) {
        const permMap: {[moduleId: number]: RolePermission} = {};
        (permRes.data || []).forEach((perm: RolePermission) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const roleRes = await userAPI.updateRole(parseInt(id), formData);
      if (!roleRes.success) {
      ...
      for (const module of modules) {
        ...
        if (perm) {
          ...
          if (perm.id) {
            ...
            await userAPI.updateRolePermission(perm.id, permData);
          } else {
      ...
      toast.error(error.message || 'Failed to update role and permissions');
    } finally {
      setLoading(false);
    }
  };
