...
import { FileText, Download, FileDown, Building2, Calendar } from 'lucide-react';
...
export default function TaxOutputPage() {
  ...
  const fetchTaxData = async () => {
    ...
    try {
      const [taxRes, companyRes] = await Promise.all([
        accountingAPI.getTaxOutput(startDate, endDate),
      ...
      if (taxRes.success) {
        ...
        setError(taxRes.message || 'Failed to load tax output data');
        console.error('Tax output API error:', taxRes);
      }
      ...
      console.error('Failed to fetch tax output:', err);
      setError('Failed to load tax output data');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'PPN Keluaran');
      ...
      XLSX.writeFile(wb, `PPN_Keluaran_${startDate}_to_${endDate}.xlsx`);
    } catch (error) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">Loading PPN Keluaran...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600 mt-1">Laporan Pajak Pertambahan Nilai Keluaran</p>
          </div>
              ...
              {taxData.length === 0 ? (
                <tr>
                  <td colSpan={7} className="px-6 py-8 text-center text-gray-500">
                    Tidak ada data PPN Keluaran untuk periode yang dipilih
                  </td>
                </tr>
              ) : (
