...
import { FileText, Download, FileDown, Building2, Calendar, TrendingUp } from 'lucide-react';
...
export default function SalesReportPage() {
  ...
  const fetchSalesData = async () => {
    ...
    try {
      const [salesRes, companyRes] = await Promise.all([
        accountingAPI.getTaxOutput(startDate, endDate), // Using tax output as it contains sales data
      ...
      if (salesRes.success) {
        ...
        console.error('Sales API error:', salesRes);
      }
  ...
  const paidSales = salesData.filter(item => item.status === 'paid').reduce((sum, item) => sum + (item.total || 0), 0);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">Loading Laporan Penjualan...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="p-6">
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600 mt-1">Ringkasan penjualan dan faktur penjualan</p>
          </div>
          <div className="flex gap-2">
            <button
              onClick={handleExportExcel}
              disabled={exportLoading || salesData.length === 0}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-green-50 rounded-lg shadow p-4">
          <p className="text-sm text-green-700">Penjualan Lunas</p>
          <p className="text-2xl font-bold text-green-900">
            Rp {paidSales.toLocaleString('id-ID')}
          </p>
        </div>
        <div className="bg-red-50 rounded-lg shadow p-4">
          <p className="text-sm text-red-700">Penjualan Belum Lunas</p>
          <p className="text-2xl font-bold text-red-900">
            Rp {unpaidSales.toLocaleString('id-ID')}
          </p>
        </div>
      </div>
