...
export default function SalesInvoiceDetailPage() {
  ...
  const handlePrint = useReactToPrint({
    contentRef: printRef,
    documentTitle: `Sales-Invoice-${invoice?.invoice_no}`,
  });
  ...
  const fetchInvoiceDetail = async () => {
    ...
    try {
      ...
      const [invoiceRes, companyRes] = await Promise.all([
        fetch(`http://localhost:8080/api/sales-invoices/${params.id}`, {
          headers: {
      ...
      <div ref={printRef} className="max-w-4xl mx-auto bg-white shadow-lg rounded-xl p-8">
        ...
        <div className="grid grid-cols-2 gap-6 mb-6 text-sm">
          ...
          <div>
            ...
            <p className="font-medium text-gray-900">{invoice.customer_name || `Customer #${invoice.customer_id}`}</p>
