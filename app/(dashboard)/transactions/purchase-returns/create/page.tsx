...
export default function CreatePRPage() {
  ...
  const fetchPaidPIs = async () => {
    try {
      const result = await masterAPI.getPurchaseInvoices(1, 100, 'paid');
      setPIs(result.data?.data || result.data || []);
    } catch (error) {
  ...
  const handlePISelect = (piId: string) => {
    const pi = pis.find(p => p.id === parseInt(piId));
    setSelectedPI(pi || null);

    if (pi) {
      const prItems: PRItem[] = pi.details.map(detail => ({
        item_id: detail.item.id,
        item_name: detail.item.name,
        item_code: detail.item.code,
        item_unit: detail.item.unit,
        available_qty: detail.qty, // This would be actual stock, but for now using ordered qty
        return_qty: 0,
        price: detail.price,
        subtotal: 0,
        batch_no: '',
  ...
  const updateItemField = (index: number, field: keyof PRItem, value: string | number) => {
    const newItems = [...items];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await masterAPI.createPurchaseReturn(payload);
      if (!(res && res.success)) {
        throw new Error(res?.message || 'Failed to create purchase return');
      }

      toast.success('Purchase return created successfully!');
      router.push('/transactions/purchase-returns');
    } catch (error: any) {
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div>
        ...
        <h1 className="text-3xl font-bold text-gray-900">Create Purchase Return</h1>
        <p className="text-gray-600 mt-1">Create retur pembelian for supplier</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
              ...
              >
                ...
                {pis.map(pi => (
                  <option key={pi.id} value={pi.id}>
                    {pi.code} - {pi.supplier.name} (Rp {pi.total_after_tax?.toLocaleString('id-ID')})
                  </option>
                ))}
              </select>
        ...
        <div className="flex justify-end gap-4">
          ...
          <button
            type="submit"
            disabled={loading || !selectedPI || items.length === 0}
