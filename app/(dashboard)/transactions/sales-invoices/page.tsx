...
export default function SalesInvoicesPage() {
  ...
  const fetchInvoices = async (applyFilters = false) => {
    try {
      ...
      const result = await masterAPI.getSalesInvoices(1, 100, statusParam, shouldApplyFilters ? fromDate : undefined, shouldApplyFilters ? toDate : undefined);
  ...
  const filteredInvoices = invoices.filter(inv => {
    const matchesSearch =
      ...
      inv.sales_order?.code?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      inv.sales_order?.customer?.name?.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600 mt-1">Manage customer invoices for payment collection</p>
        </div>
        <Link
          href="/transactions/sales-invoices/create"
      ...
      <div className="bg-white p-4 rounded-lg shadow">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            <input
              type="text"
              placeholder="Search by invoice code, SO code, or customer..."
              value={searchTerm}
              ...
              {filteredInvoices.length === 0 ? (
                <tr>
                  <td colSpan={8} className="px-6 py-12 text-center text-gray-500">
                    {searchTerm || statusFilter !== 'all' ? 'No sales invoices found matching your filters' : 'No sales invoices yet. Create your first invoice!'}
                  </td>
                </tr>
              ) : (
                filteredInvoices.map((inv) => (
                  <tr key={inv.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="text-sm text-gray-900">{inv.sales_order?.code || '-'}</span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="text-sm text-gray-900">{inv.sales_order?.customer?.name || '-'}</span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                      <button
                        onClick={() => router.push(`/transactions/sales-invoices/${inv.id}`)}
                      ...
                      <button
                        onClick={() => router.push(`/transactions/sales-invoices/${inv.id}/print`)}
        ...
        {filteredInvoices.length > 0 && (
          <div className="bg-gray-50 px-6 py-4 border-t border-gray-200">
            <span className="text-sm text-gray-600">
              Showing {filteredInvoices.length} of {invoices.length} sales invoices
            </span>
          </div>
        )}
      </div>
