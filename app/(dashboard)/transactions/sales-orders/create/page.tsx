...
export default function CreateSalesOrderPage() {
  ...
  const fetchMasterData = async () => {
    ...
    try {
      // Fetch customers, warehouses, items and active tax using API helpers
      const customersData = await masterAPI.getCustomers(1, 100);
  ...
  const handleAddItem = (item: Item) => {
    const existing = soItems.find(i => i.item_id === item.id);
    if (existing) {
  ...
  // Calculate totals
  const subtotal = soItems.reduce((sum, item) => sum + item.subtotal, 0);
  ...
  const handleSubmit = async (status: 'draft' | 'submitted') => {
    ...
    try {
      ...
      const data = await masterAPI.createSalesOrder(payload);

      if (data && data.success) {
        success(`Sales order ${status === 'draft' ? 'saved as draft' : 'submitted'} successfully!`);
        setTimeout(() => {
          router.push('/transactions/sales-orders');
        }, 1500);
      } else {
        showError((data && data.message) || 'Failed to create sales order');
      }
    } catch (err: any) {
    ...
    <>
      ...
      <div className="p-6 max-w-6xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-600 mt-1">Create a new sales order to customer</p>
        </div>
                      ...
                      >
                        <div>
                          ...
                          <div className="text-sm text-gray-500">
                            Code: {item.code} | Unit: {item.unit} | Stock: {item.stock}
                          </div>
                        </div>
